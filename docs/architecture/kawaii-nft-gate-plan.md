...
- ✅ **A.3 — compose + pin** (commit `ef04305`): sharp composer (`lib/kawaii/{layers,compose}.ts`, path-traversal allowlist, reads `KAWAII_LAYERS_DIR`), Pinata pin → CIDv1 (`pin.ts`), ERC-1155 metadata + reserved attestation (`metadata.ts`). Compose smoke-tested (base ✓). **Assets need alpha cutout** (trait layers are RGB w/ baked checkerboard — the photoshop pass); bases are RGBA-clean.
- ✅ **A.4 — mint service + route** (`ef04305`): `mint-service.ts` 6-step pipeline (reserved gate owner-only/mock-refuse/mint-once, server compose+pin CIDv1, Circle `mintTo` sentinel gas-sponsored, DB record + `baseId` col). Route `POST /api/kawaii/mint` (SIWE wallet-sig auth, rejects to/uri/cid/tokenId, socials gate, whitelist→free | 402).
- ⚠️ **Wiring needed to RUN A.4:** add to `apps/web` env (from sendero) — `PINATA_JWT`, `CIRCLE_API_KEY`, `CIRCLE_ENTITY_SECRET`, `KAWAII_ATTEST_SECRET`; set reserved base art; production layer hosting (151M not committed).
- ✅ **A.7 — gate modal + wiring + QA** (commits `1dd6b5c`, `98b2d7c`): `KawaiiGate` (Tower-style, matches app UX — Knicknack headline, dark/violet card, white CTA, socials, base picker w/ 4 reserved LOCKED, nanopay advert). `KawaiiGateMount` self-gates (connected + no NFT, fails open). `/api/kawaii/catalog` (open+reserved, no owner wallets) + `/api/kawaii/status` (hasNft from ledger). Mounted in `home/index.tsx`. browse-QA'd at `/kawaii-preview` — renders correctly.
- ✅ **A.4 RUN** — real mint verified end-to-end (token 1 → KEEPER, tx `0x248de5cc…`); creds set local + Vercel.
- ✅ **A.8 — payment verify** (commit `0686519`): TESTNET = **USDC on Arc only** (JPYC + −20% are mainnet/Avax). `verifyUsdcPaymentArc` (ERC20 Transfer 6-dec OR native value 18-dec → agent, from payer, ≥100 USDC); tested vs a real payment (tx `0xc06f70c2…`) — exact verified, underpay/wrong-recipient rejected. Route: non-whitelist requires `paymentTx` → dedup (unique col) → verify → mint. Gate UX pays USDC → waits → resubmits.
- ✅ **A.6 — social OAuth** (commit `3c73772`): Discord + X redirect OAuth (X w/ PKCE), Telegram Login-Widget verify; guild-membership / follow / chat-member checks (when env set); `/social/status` drives the gate ticks; state HMAC-bound to wallet (tested). **NEEDS ENV to activate:** `APP_URL`, `DISCORD_CLIENT_ID`/`_SECRET`/`_GUILD_ID`, `X_CLIENT_ID`/`_SECRET`/`_FOLLOW_TARGET_ID`, `TELEGRAM_BOT_TOKEN`/`_CHAT_ID`. Remaining UI: embed the TG Login Widget in the gate.
- ✅ **A.9 — fund panel** (commit `92107db`): `KawaiiFund` shows USDC balance on Arc vs 100 needed + faucet link (testnet need). Full USDC→JPYC swap (−20%) + Avax bridge deferred to mainnet tier (reuses desk-v1 AppKit swap + TokenChip + BlockchainIcons). browse-QA'd.
- ⏭ Next: register the 3 social apps + add creds (activates A.6); base-thumbnail rendering + reserved base art; prod layer hosting; TG login-widget embed; MCP mint tool (EIP-1271 agent SCAs); asset alpha cutout; **mainnet tier** (deploy on Avax + USDC/JPYC swap+bridge + upgrade flow + leaderboard).

## Testnet flow status — every layer built + verified
connect → socials (A.6, needs creds) → fund (A.9) → pick base → pay 100 USDC (A.8, on-chain verified) → mint (A.4, proven on-chain, token 1→KEEPER) → gate wired (A.7, browse-QA'd). NFT contract verified 4/4. Remaining = activation (social creds, prod layer hosting) + polish (thumbnails, reserved art, TG widget) + the separate mainnet tier.
