...
import { hubByChainId } from "./chains";
import { getHubClient, type HubClientMap } from "./clients";
import { WAD } from "./constants";
import { FxTelaranaError } from "./errors";
import { getMarketById, listMarkets, readMarketState } from "./market-view";
import { MorphoBlueAbi } from "./morpho-blue-abi";
import { toAssetsDown, toAssetsUp } from "./morpho-math";
import { readFxOracleMid } from "./oracle";
import { calculateHealthFactorE18 } from "./quote-engine";
import type {
  AccountPosition,
  LendingMarket,
  MorphoMarketState,
...
export async function readMorphoPosition(args: {
  ...
  })) as MorphoPositionState;
  return {
    supplyShares: BigInt(raw.supplyShares),
    borrowShares: BigInt(raw.borrowShares),
...
export async function getAccountPosition(args: {
  ...
  const oracle = needsOracle
    ...
    : undefined;
  return buildAccountPositionView({
    market,
    state,
    position,
    account: args.account,
...
export function assertMarketStateAvailable(
  state: MorphoMarketState | undefined,
): asserts state is MorphoMarketState {
  if (!state) {
    throw new FxTelaranaError(
      "Morpho market state is unavailable",
