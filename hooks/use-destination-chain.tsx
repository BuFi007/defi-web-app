...
export const useDestinationToken = () => {
  const getDestinationTokenAddress = useCallback((
    ...
    try {
      ...
      const destinationTokens = useGetTokensOrChain(Number(destinationChainId), "tokens");
      ...
      if (Array.isArray(destinationTokens)) {
        matchingToken = destinationTokens.filter((token: Token) => token.symbol === tokenSymbol);
      } else {
        console.warn(`Destination tokens is not an array for chain ${destinationChainId}`);
        return NATIVE_TOKEN_ADDRESS;
      }
      
      if (!matchingToken.length) {
        console.warn(`No matching token found for ${tokenSymbol} on chain ${destinationChainId}`);
        return NATIVE_TOKEN_ADDRESS;
      }
