...
| 5.2.5 | Matcher can read `FxOracle.getMid` without revert. Phase 7.2's `pyth_pusher` task pushes Hermes VAA updates every `PYTH_PUSH_INTERVAL_MS` (default 5s) for every feed referenced by `MATCHER_FUNDING_MARKET_IDS`. Verified live on Arc 2026-05-23: first tick pushed 4 feeds (USDC + 3 base tokens) in one tx; oracle gate passes thereafter. Same fix unblocks BUFX `FxSpotExecutor`. | ✅ — Phase 7.2 shipped |
