...
| Semantic | `tools/call` arguments nest under `body` (`{"arguments":{"body":{…}}}`) — undiscoverable from `tools/list` | FIXED + LIVE — inputSchema nests fields under `body`; a flat-args `tools/call` returns a `-32602` naming the `body` wrapper + the exact fields to move | `mcp/server.ts` `callTool` envelope check |
| Semantic | Path params dropped from MCP `inputSchema` for ALL path-param GET tools (positions/portfolio/lending-positions/reputation check·score·identity/copy status·leader) → client never sends the address; `:param` leaks into upstream path | FIXED local (broad run 2026-05-29, not yet deployed) — `toMCPManifest` derives `:param` from path → `params.properties.{name}` (required, typed, hints for address/follower/agentId) | `hyper/core/projection.ts` `paramsProperty()` |
| Semantic | 0/70 OpenAPI operations carried `summary`/`description` (prose lived only in `tools/list`) | FIXED local — `buildOperation` projects route MCP `title`/`description` into the op; now 66/70 carry descriptions; `info.description` set | `hyper/openapi/generate.ts`, `app.ts` openapiHandlers |
| Semantic | `llms.txt` "always trader" rule actively misled — ~9 endpoints 400 on `trader` (real names: `lp`, `follower`/`leader`, `address`, `raterWalletUuid`) | DOC FIXED local — rule scoped to spot/perp/lending/ghost + per-family names listed | `app.ts` `llmsTxt`; true fix = accept `trader` alias on lp/copy/bonds too |
| Semantic | Whole families absent from `llms.txt` (copy-trading, bonds/ERC-8183, LP/TurboFeeVault, fxswap cross-currency, hedge, oracle/registry/gateway) | FIXED local — sections + FX-swap matrix row added (11.7KB→17.9KB) | `app.ts` `llmsTxt` |
| Semantic | Stale `bufi_*` tool-name breadcrumbs in `note`/description fields point at nonexistent tools | FIXED local — 21 refs swept to real `post__api_*`/`get__api_*` names; 0 remain | `routes/{bonds,copy-trading,lending,reputation,spot,trade}.ts` |
| Semantic | ~37 ops declare no 200 response schema (trade/prepare, quote, cost, spot/buy, portfolio/positions reads, copy/bonds/reputation) | OPEN | add `.output(zodSchema)` to non-prepare money routes (pattern already on 33 ops) |
| Execution | Borrow EXECUTE returned 500 "quote reader not configured" | FIXED + LIVE | local borrow quote reader in `services.ts` |
| Execution | `lending/borrow/preview` still returns `{error:"borrow preview unavailable"}` on prod (2026-05-29) for valid input — distinct from borrow-execute above | OPEN (disclosed as KNOWN LIMITATION in `llms.txt` this run) | wire the on-chain quote reader for the PREVIEW path |
| Execution | `lending/*-prepare` rejects EVERY marketId from `/api/lending/markets` ("not found on any Arc Morpho") — markets lists Fuji-hub (43113), prepare resolves Arc (5042002) only | OPEN (disclosed in `llms.txt`) | unify the marketId universe — prepare resolves the Fuji-hub ids markets returns, or markets emits Arc ids |
| Execution | `bonds/create`+`stake` are non-executable stubs (bondId + prose, no escrow addr/calldata/approve) | OPEN (disclosed as off-chain registry stub in `llms.txt`) | return prepared calldata + ERC-8183 escrow address + approve preflight, or keep stub framing + publish the escrow address |
