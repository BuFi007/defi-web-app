...
import { MAX_INTENT_DEADLINE_SECONDS } from "./constants";
...
export function buildBorrowIntent(
  input: BaseIntentInput & { borrowAssets: bigint; receiver: Address },
) {
  return {
    domain: domainFor(input.chainId),
...
export function buildWithdrawIntent(
  input: BaseIntentInput & { shares: bigint; receiver: Address },
) {
  return {
    domain: domainFor(input.chainId),
...
// WithdrawCollateral uses the same Collateral typed-data shape as supply.
